...
function amshopby_ajax_request(url){
	...
	if (amshopby_use_hash) {
		...
		if (pos != -1 && pos != url.length - 1) {
			...
			if (typeof amscroll_object != 'undefined') {
				var tmpUrl = window.top.location.protocol + '//' + window.top.location.host + window.top.location.pathname + '?' + hash;
			 ...
			 * SEO mode
			 */
			var l = amshopby_get_location_from_string(url);
			var pathname = l.pathname;
			
			/*
			 * IE hack for pathname
			 * as it's returned as index.php instead of /index.php
			 */
			if (pathname.indexOf('/') != 0) {
				pathname = '/' + pathname;
			}
			
			window.top.location.hash = pathname; 	
			...
			if (typeof amscroll_object != 'undefined') {
				...
				/*
				 * Remove everything after = sign in hash
				 */
				if (hash.indexOf('=') != -1) {
				...
				var tmpUrl = window.top.location.protocol + '//' + window.top.location.host + hash;
...
document.observe("dom:loaded", function(event) {
	...
	if (amshopby_request_required()) {
		...
		if (hash.indexOf('/') == 0) {
			 ...
			 */
			if (hash.indexOf('=') != -1) {
				url = url + hash.substr(0, hash.indexOf('='));
			} else {
				url = url + hash;	
			}
		} else {
			 ...
			 * Ordinary mode
			 */
			url = url + window.top.location.pathname + '?' + hash;
		}
