...
function initOpenClose() {
	jQuery('ul.add-nav li').openClose({
...
function initAccordion() {
	jQuery('ul.accordion-2').slideAccordion({
		opener: 'a.opener',
		slider: 'div.slide',
		animSpeed: 300
	});
...
// popups init
function initPopups() {
	jQuery('ul.list li').contentPopup({
		mode: 'hover'
	});
	jQuery('.item').contentPopup({
		mode: 'click',
		popup: '.open-block',
		btnOpen: '.opener',
		btnClose: '.close-2',
...
;(function($) {
	...
	ContentPopup.prototype = {
		init: function() {
			this.findElements();
			this.attachEvents();
			this.attachEventsCloser();
		},
		findElements: function() {
			this.popup = this.holder.find(this.options.popup);
			this.btnOpen = this.holder.find(this.options.btnOpen);
			this.btnClose = this.holder.find(this.options.btnClose);
			if(this.options.ajaxMode) this.btnOpen.data('inc', this.btnOpen.attr('rel')).removeAttr('rel');
		},
		attachEvents: function() {
		...
		showPopup: function() {
			var self = this;
			if(this.btnOpen.data('inc')) {
			jQuery.ajax({
				type: 'get',
				url: self.btnOpen.data('inc'),
				dataType: 'html',
				success: function(popupHTML){
					self.btnOpen.removeData('inc');
					self.popup[0].innerHTML = popupHTML;
					jcf.customForms.replaceAll();
					self.btnClose = self.holder.find(self.options.btnClose);
					self.attachEventsCloser();
					self.loadedShow();
				},
				error: function(){
					alert('AJAX Error!');
