...
(function (window, undefined) {
    ...
    function processResponse(request) {
        ...
        if (request.response.Items.length === 1 && request.response.Items[0].Error !== undefined)
           {
		   		pcaValid = false;
				jQuery("#zip").removeClass('validate-zip-international').removeClass('validation-failed');
				jQuery("input[name=billing\\[postcode\\]]").removeClass('validate-zip-international-billing').removeClass('validation-failed');
				jQuery("input[name=shipping\\[postcode\\]]").removeClass('validate-zip-international-shipping').removeClass('validation-failed');
    ...
    pca.AutoComplete = function (fields, options) {
        ...
        autocomplete.attach = function (field) {
            ...
            function positionAdjacentField(f) {
                ...
                //BA-364 disable company from check
                if (f.title == 'Company') return;
